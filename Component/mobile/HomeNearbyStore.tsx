...
const HomeNearbyStore = ({ passLocationData }: any) => {
    ...
    // console.log(data?.map((e: any) => e?.ShopName));
    // console.log(data);
        ...
        <>
            {data?.length !== 0 && <div className={style.best_seller}>
                <div className={style.best_seller_title}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: '5px' }}> <p>Near by Store</p>
                        <p onClick={() => router.push('/nearbystore')} style={{ fontSize: '12px' }}>view all {`>`}</p>
                    </div>
                </div>
                            ...
                            return (
                                <>
                                    ...
                                    <div onClick={() => handelClickForVisitShop(e?.UserSite)} key={index} className={style.cardSingle}>
                                        ...
                                        {/* <p className={style.AddressForBestSeller}>{e?.Address}</p> */}
...
export default HomeNearbyStore;